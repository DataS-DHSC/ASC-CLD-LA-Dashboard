...
AS
      ...
      CASE WHEN @Submissions = 'Single' THEN (
        ROW_NUMBER() OVER (
          PARTITION BY
            ...
            (CASE WHEN Event_Type like '%service%' THEN ISNULL(Delivery_Mechanism, '') END),
            (CASE WHEN Event_Type like '%service%' THEN ISNULL(Unit_Cost, '') END),
            (CASE WHEN Event_Type like '%service%' THEN ISNULL(Cost_Frequency_Unit_Type, '') END),
  ...
  BEGIN
    ...
    ALTER TABLE #OutputTable DROP COLUMN Cost_Frequency_Unit_Type;
