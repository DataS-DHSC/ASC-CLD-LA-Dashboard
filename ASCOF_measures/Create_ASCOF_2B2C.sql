...
SELECT 
  ...
  Der_NHS_LA_Combined_Person_ID,
  MIN(CASE 
        WHEN Der_Age_Event_Start_Date BETWEEN 18 AND 64 THEN '18 to 64'
        WHEN Der_Age_Event_Start_Date > 64 then '65 and above'
        ELSE NULL
      END) AS Age_Band
INTO #Deduplicated
FROM #New_Admissions
WHERE Der_Age_Event_Start_Date >= 18
GROUP BY 
  LA_Code,
  LA_Name,
  Der_NHS_LA_Combined_Person_ID;
...
DROP TABLE IF EXISTS ASC_Sandbox.ASCOF_2BC_Revised;
