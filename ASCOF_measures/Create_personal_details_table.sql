...
This script is set up to run on the release 2 specification (data submitted under R1 is mapped to R2 when the main tables are produced). 
*/
...
DROP PROCEDURE IF EXISTS ASC_Sandbox.Create_person_details_table

GO

CREATE PROCEDURE ASC_Sandbox.Create_person_details_table
...
AS
  ...
  EXEC(@Query)
    ...
    --Pull through required fields and map any nulls to unknown
    --Input table should be the latest joined submission table according to Release 2
    ...
    SELECT 
      ...
      COALESCE(a.Ethnicity_Cleaned, 'Invalid and not mapped') AS Ethnicity_Cleaned,
      COALESCE(a.Ethnicity_Grouped, 'Invalid and not mapped') AS Ethnicity_Grouped,
      COALESCE(a.Client_Funding_Status_Cleaned, 'Invalid and not mapped') AS Client_Funding_Status_Cleaned,
      COALESCE(a.Employment_Status_Cleaned, 'Invalid and not mapped') AS Employment_Status_Cleaned,
    ...
    INTO #CLD
    FROM ASC_Sandbox.InputTable a  --Input table
    LEFT JOIN ASC_Sandbox.REF_Has_Unpaid_Carer_Mapping uc  --The only field not cleaned in the main joined submission table
    ...
    DROP TABLE IF EXISTS #REF_Accommodation_Status;
        ...
        VALUES
          ...
          ('Rough sleeper or squatting',0,10),
          ('Night shelter, emergency hostel or direct access hostel',0,11),
          ('Refuge',0,12),
          ('Placed in temporary accommodation by the council (inc. homelessness resettlement)',0,13),
          ('Staying with family or friends as a short term guest',0,14),
          ('Acute hospital or long term healthcare residential facility',0,15),
          ('Registered care home',0,16),
          ('Registered nursing home',0,17),
          ('Prison, young offenders institution or detention centre',0,18),
    ...
    VALUES
      ...
      ('Not in paid employment: Not actively seeking work or retired', 0, 2),
            ...
            *,
            DENSE_RANK() OVER ( --services with the same dates, service type hierarhcy, or one of the 3 service components which can be mapped are given the same row number
              ...
              ORDER BY
                      ...
                      CASE 
                        WHEN Service_Component_Cleaned IN ('Shared Lives', 'Community supported living', 'Extra care housing', 'Home support or domiciliary care') 
                        THEN 1 ELSE 0 
    ...
    FROM (
      SELECT
        ...
        CASE 
          ...
          WHEN Service_Component = 'Community supported living' THEN 'Supported accommodation, supported lodgings or supported group home'
          WHEN Service_Type_Cleaned = 'Long term support: Community' AND  Service_Component = 'Home support or domiciliary care' THEN 'Unknown: Presumed at home'-- new constructed category: not in spec
          ...
          WHEN Service_Type_Cleaned = 'Long term support: Prison' THEN 'Prison, young offenders institution or detention centre'
          ELSE 'Unknown'
        END AS Accommodation_Status
    ...
    INTO #Carers_shell
    FROM #CLD
    WHERE Client_Type_Cleaned IN ('Unpaid carer', 'Unpaid carer known by association')
    ...
    SELECT
      ...
      CASE
        WHEN Primary_Support_Reason_Cleaned IN ('Unknown','Invalid and not mapped','Social support: Support to unpaid carer') OR -- this deprioritises 'Social support: Support to unpaid carer'
    ...
    DECLARE @CLD_Count INT;
    DECLARE @OutputTable_Distinct_Count INT;
    DECLARE @OutputTable_Row_Count INT;

    -- Get count of distinct (LA_Code, Der_NHS_LA_Combined_Person_ID) pairs in #CLD
    SELECT @CLD_Count = COUNT(*)
    FROM (
    ...
    -- Get count of distinct (LA_Code, Der_NHS_LA_Combined_Person_ID) pairs in #OutputTable
    SELECT @OutputTable_Distinct_Count = COUNT(*)
    FROM (
        SELECT LA_Code, Der_NHS_LA_Combined_Person_ID
        FROM #OutputTable
        GROUP BY LA_Code, Der_NHS_LA_Combined_Person_ID
    ...
    -- Get total row count in #OutputTable
    SELECT @OutputTable_Row_Count = COUNT(*) FROM #OutputTable;

    -- Compare and print result
    IF @CLD_Count = @OutputTable_Distinct_Count AND @OutputTable_Distinct_Count = @OutputTable_Row_Count
        PRINT 'ALL OK - Row count matches distinct LA Code and Person ID combinations';
    ELSE
        PRINT 'ISSUE - Row count not equal to the number of distinct LA Code & Person ID combinations';
...
EXEC ASC_Sandbox.Create_person_details_table
  ...
  @InputTable = 'ASC_Sandbox.CLD_230401_250630_JoinedSubmissions_r2_test',
  @OutputTable = 'ASC_Sandbox.CLD_230401_250630_JoinedSubmissions_Latest_Person_Data_R2_2425_test'
*/