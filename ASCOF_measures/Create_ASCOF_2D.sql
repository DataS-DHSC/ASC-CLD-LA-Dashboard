...
DROP PROCEDURE IF EXISTS ASC_Sandbox.Create_ASCOF2D

GO

CREATE PROCEDURE ASC_Sandbox.Create_ASCOF2D
 --Dates
...
--Inputs
  ...
  @InputTable_SUS AS NVARCHAR(100),
--Outputs
  @OutputTable_Part1 AS NVARCHAR(100),
  @OutputTable_Part1_Demographics AS NVARCHAR(100),
  @OutputTable_Part2 AS NVARCHAR(100),
  @OutputTable_Part2_Demographics AS NVARCHAR(100),
  @OutputTable_Venn AS NVARCHAR(100)
...
AS
  ...
  DROP SYNONYM IF EXISTS ASC_Sandbox.InputTable_SUS

  SET @Query = N'DROP TABLE IF EXISTS ' + @OutputTable_Part1 + '; 
                 DROP TABLE IF EXISTS ' + @OutputTable_Part1_Demographics + '; 
                 DROP TABLE IF EXISTS ' + @OutputTable_Part2 + '; 
                 DROP TABLE IF EXISTS ' + @OutputTable_Part2_Demographics + '; 
                 DROP TABLE IF EXISTS ' + @OutputTable_Venn + '; 
                 ...
                 CREATE SYNONYM ASC_Sandbox.InputTable_SUS FOR ' + @InputTable_SUS +';'
  EXEC(@Query)
      ...
      FROM ASC_Sandbox.InputTable_SUS A
      WHERE 
	      CASE 
		      WHEN Month_of_Birth_SUS IS NOT NULL AND Year_of_Birth_SUS IS NOT NULL 
		      THEN FLOOR((DATEDIFF(DAY, CAST(CONCAT(Year_of_Birth_SUS, '-', Month_of_Birth_SUS, '-01') AS DATE), Discharge_Date)) / 365.25)
        ...
        AND A.Der_DHSC_Pseudo_NHS_Number IS NOT NULL
	      ...
	      AND NOT EXISTS (SELECT 1 FROM ASC_Sandbox.ASCOF_2D_LTS B 
		      WHERE 
      ...
      SELECT DISTINCT
          a.*,
	      CASE
		      WHEN a.Der_Latest_Age_SUS BETWEEN 65 AND 74 THEN '65-74'
		      WHEN a.Der_Latest_Age_SUS BETWEEN 75 AND 84 THEN '75-84'
		      WHEN a.Der_Latest_Age_SUS >= 85 THEN '85+'
		      ELSE NULL
	      END AS Der_Age_Grouped,
      ...
      */
      ...
      FROM ASC_Sandbox.InputTable_SUS
      WHERE 
      ...
      ------------------------------------------------------------------------
      -- Creating a Group reference table for slicing the ASCOF population by 'gender' and 'age'
      DROP TABLE IF EXISTS #REF_Group;
      SELECT Slice, Sort_Order
      INTO #REF_Group
      FROM (VALUES
          ('Total', 1),
          ('65-74', 2),
      ...
      -- LA × Client_Type × Group reference (for zero rows)
      DROP TABLE IF EXISTS #REF_Client_Type_LA_Group;
      SELECT
          r.Area,
          r.Client_Type,
          g.Slice
      INTO #REF_Client_Type_LA_Group
      FROM #REF_Client_Type_LA r
      CROSS JOIN #REF_Group g;
      ...
      INTO #OutputTable_Part1_Demographics
      FROM #REF_Client_Type_LA_Group r
      LEFT JOIN (
          SELECT
          ...
          FROM #Discharge_With_Outcome d
          JOIN #REF_Group grp
            ON (grp.Slice = 'Total'
			      OR (grp.Slice IN ('65-74','75-84','85+') AND d.Der_Age_Grouped = grp.Slice)
			      OR (grp.Slice IN ('Female','Male') AND d.Gender = grp.Slice))
          GROUP BY ROLLUP(Area), ROLLUP(Client_Type), grp.Slice
      ) a
        ON r.Area = a.Area
       AND r.Client_Type = a.Client_Type
       AND r.Slice = a.Slice;
      ...
      INTO #Denominator
      FROM ASC_Sandbox.ASCOF_2D_All_Discharges d
      JOIN #REF_Group s
        ON (
             s.Slice = 'Total'
          OR (s.Slice IN ('65-74','75-84','85+') AND d.Der_Age_Grouped = s.Slice)
          OR (s.Slice IN ('Female','Male')        AND d.Gender = s.Slice)
        )
      GROUP BY ROLLUP(Client_Type), ROLLUP(UTLA_Name), s.Slice;


      -- Numerator: inherit from Part1 metrics
      ...
      INTO #Numerator
      FROM #REF_Client_Type_LA_Group r  -- Area × Client_Type × Slice
      LEFT JOIN #OutputTable_Part1_Demographics n
        ON r.Area = n.Area
       AND r.Client_Type = n.Client_Type
       AND r.Slice = n.Slice;
      ...
      INTO #OutputTable_Part2_Demographics
      FROM #Numerator n
      LEFT JOIN #Denominator d
        ON d.Area = n.Area
       AND d.Client_Type = n.Client_Type
       AND d.Slice = n.Slice;

      --=========================================================================================================
      --==============================ENSURING THE DASHBOARD DOESN'T BREAK=======================================
      --1. ASCOF 2D Part 1
      DROP TABLE IF EXISTS #OutputTable_Part1;
      SELECT 
	      Area,
	      Client_Type,
	      Reporting_Period,
	      ...
	      Readmitted,
	      Entered_LT_ResNurs
      INTO #OutputTable_Part1	
      FROM #OutputTable_Part1_Demographics
      WHERE Slice = 'Total'
      ...
      SELECT 
	      ...
	      Reablement_Count, 
	      Outcome_Percent
      INTO #OutputTable_Part2
      FROM #OutputTable_Part2_Demographics
      WHERE Slice = 'Total'
      ...
      */
    --Tidy up
    DROP TABLE IF EXISTS ASC_Sandbox.ASCOF_2D_All_Discharges
    DROP TABLE IF EXISTS ASC_Sandbox.ASCOF_2D_All_Events
    DROP TABLE IF EXISTS ASC_Sandbox.ASCOF_2D_CLD_Cleaned
    DROP TABLE IF EXISTS ASC_Sandbox.ASCOF_2D_Discharge_Reablement_Outcomes
    DROP TABLE IF EXISTS ASC_Sandbox.ASCOF_2D_Discharges_Into_Reablement
    DROP TABLE IF EXISTS ASC_Sandbox.ASCOF_2D_LTS
    DROP TABLE IF EXISTS ASC_Sandbox.ASCOF_2D_Reablement


    SET @Query = 'SELECT * INTO ' + @OutputTable_Part1 + ' FROM #OutputTable_Part1
                  SELECT * INTO ' + @OutputTable_Part1_Demographics + ' FROM #OutputTable_Part1_Demographics
                  SELECT * INTO ' + @OutputTable_Part2 + ' FROM #OutputTable_Part2
                  SELECT * INTO ' + @OutputTable_Part2_Demographics + ' FROM #OutputTable_Part2_Demographics
                  SELECT * INTO ' + @OutputTable_Venn + ' FROM #OutputTable_Venn'
    EXEC(@Query)
...
--Inputs
  ...
  @InputTable_SUS = 'DHSC_Reporting.ASCOF2D_SUS_250908',  --should be updated with sandbox table made in previous step
--Outputs
  @OutputTable_Part1 = 'ASC_Sandbox.ASCOF_2D_Part1_Latest',
  @OutputTable_Part1_Demographics = 'ASC_Sandbox.ASCOF_2D_Part1_Demographics_Latest',
  @OutputTable_Part2 = 'ASC_Sandbox.ASCOF_2D_Part2_Latest',
  @OutputTable_Part2_Demographics = 'ASC_Sandbox.ASCOF_2D_Part2_Demographics_Latest',
  @OutputTable_Venn = 'ASC_Sandbox.ASCOF_2D_Venn_Latest' 
  */
