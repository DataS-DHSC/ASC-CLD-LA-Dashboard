------  RESTRICTED SHORT TERM WAITING TIMES TABLE ------
-- This code outputs the Short_Term_Waiting_Times_Restricted table
-- It filters the previously created tables for the dashboard to find new client's first short term service
-- Where the client has a short term assessment before the service it will calculate the time between the service and that assessment and the request before that
...
INNER JOIN #Short_Term_Clients AS STC
	ON STC.Der_NHS_LA_Combined_Person_ID = ASS.Der_NHS_LA_Combined_Person_ID
  AND STC.LA_Code = ASS.LA_Code
	AND STC.First_Service >= ASS.Event_Start_Date
WHERE ASS.assessment_Type LIKE '%Short Term%'
...
DROP TABLE IF EXISTS ASC_Sandbox.LA_PBI_Short_Term_Waiting_Times_Restricted;
...
INTO ASC_Sandbox.LA_PBI_Short_Term_Waiting_Times_Restricted
